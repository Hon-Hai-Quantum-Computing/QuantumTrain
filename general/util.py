...
class LewHybridNN(nn.Module):
    def __init__(self, model, MappingNetwork, QuantumNN, nw_list_normal, network_config, n_qubit ,device):
        """
        """
        super().__init__()
        self.model = model
        self.MappingNetwork = MappingNetwork
        self.QuantumNN = QuantumNN
    ...
    def forward(self, x):
        ...
        """
        device = x.device
        prob_len = len(self.nw_list_normal)
        probs_ = self.QuantumNN()
        probs_ = probs_[:prob_len]

        n_qubit = self.n_qubit
    
        # Generate qubit states using PyTorch
        qubit_states_torch = generate_qubit_states_torch(n_qubit)[:prob_len]
        qubit_states_torch = qubit_states_torch.to(device)
        # Combine qubit states with probability values using PyTorch
    
        combined_data_torch = torch.cat((qubit_states_torch, probs_), dim=1)
        combined_data_torch = combined_data_torch.reshape(prob_len, 1, n_qubit+1)
    ...
    def QuantumTrain(model, n_qubit, nw_list_normal, q_depth, device, network_config):
        MappingNetwork = LewHybridNN.MappingModel(n_qubit+1, [4, 20, 4], 1).to(device)  
        QuantumNN = LewHybridNN.QLayer(q_depth, nw_list_normal).to(device)   #arch={"n_blocks": q_depth})
  
        return LewHybridNN(model, MappingNetwork, QuantumNN, nw_list_normal, network_config, n_qubit, device)

    class QLayer(nn.Module):
        def __init__(self, n_blocks, nw_list_normal=None):
            super().__init__()
        ...
        def forward(self):
            ...
            state_mag = qdev.get_states_1d().abs()[0] 
            state_mag = state_mag[:len(self.nw_list_normal)]
            x = torch.abs(state_mag) ** 2
            # x = torch.log(x)
            x = x.reshape(len(self.nw_list_normal),1)
            x = (beta*torch.tanh(gamma*easy_scale_coeff*x))**(alpha) 
            x = x - torch.mean(x)
            x.to(self.device)
            return x
